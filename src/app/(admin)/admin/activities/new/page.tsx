...
export default function NewActivityPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { activity_type: 'sightseeing', difficulty_level: 'easy', is_active: true, max_group_size: 10, duration_hours: 3 },
  })
  ...
  const mutation = useMutation({
    mutationFn: (values: FormData) => {
      ...
      return activitiesApi.create(fd)
    },
    onSuccess: () => {
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="font-display text-2xl font-bold text-white flex items-center gap-2">
              <Zap className="h-5 w-5 text-brand-wine" /> Nueva actividad
            </h1>
          </div>
        </div>
      </div>
          ...
          <div className="rounded-2xl bg-brand-dark border border-brand-steel/10 p-5 space-y-3">
            ...
            <div onClick={() => imgRef.current?.click()}
              ...
              {preview
                ...
                : <div className="flex flex-col items-center gap-2 text-brand-steel"><ImageIcon className="h-8 w-8" /><p className="text-sm">Subir imagen</p></div>
              }
          ...
          <div className="rounded-2xl bg-brand-dark border border-brand-steel/10 p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <select {...register('activity_type')} className={`w-full rounded-xl px-3 py-2 text-sm ${fc}`}>
                  {ACTIVITY_TYPES.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
                </select>
              </div>
              <div className="space-y-1.5">
                <Label className="text-brand-silver text-xs">Dificultad</Label>
                <select {...register('difficulty_level')} className={`w-full rounded-xl px-3 py-2 text-sm ${fc}`}>
              ...
              <div className="space-y-1.5">
                ...
                <Input type="number" step="0.5" {...register('duration_hours')} className={fc} />
              </div>
              <div className="space-y-1.5">
                <Label className="text-brand-silver text-xs">Precio por persona (USD)</Label>
                <Input type="number" step="0.01" {...register('price_per_person')} className={fc} />
                {errors.price_per_person && <p className="text-red-400 text-xs">{errors.price_per_person.message}</p>}
              </div>
              <div className="space-y-1.5">
                <Label className="text-brand-silver text-xs">Tamaño máximo de grupo</Label>
                <Input type="number" {...register('max_group_size')} className={fc} />
              </div>
            </div>
            <div className="space-y-1.5">
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              ...
              <span className="text-sm text-brand-silver">Actividad activa (visible en el sitio)</span>
            </label>
          </div>
