...
export default function NewDestinationPage() {
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { continent: 'América del Sur', is_popular: false },
  })
  ...
  const mutation = useMutation({
    mutationFn: (values: FormData) => {
      ...
      return destinationsApi.create(fd)
    },
    onSuccess: () => {
    ...
    onError: () => toast.error('Error al crear el destino'),
  })
    ...
    <div className="min-h-screen bg-brand-darkest">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <h1 className="font-display text-2xl font-bold text-white flex items-center gap-2">
              <MapPin className="h-5 w-5 text-brand-wine" /> Nuevo destino
            </h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-6">
        <form onSubmit={handleSubmit(v => mutation.mutate(v))} className="space-y-6">
          ...
          <div className="rounded-2xl bg-brand-dark border border-brand-steel/10 p-5 space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input {...register('name')} className={fc} placeholder="ej. Machu Picchu" />
                {errors.name && <p className="text-red-400 text-xs">{errors.name.message}</p>}
              </div>
              <div className="space-y-1.5">
                <Label className="text-brand-silver text-xs">País *</Label>
                <Input {...register('country')} className={fc} placeholder="ej. Perú" />
              ...
              <div className="space-y-1.5">
                ...
                <Input {...register('best_season')} className={fc} placeholder="ej. Junio – Agosto" />
              </div>
            </div>
          ...
          {/* Coordenadas */}
          <div className="rounded-2xl bg-brand-dark border border-brand-steel/10 p-5 space-y-4">
            <p className="text-brand-steel text-xs uppercase tracking-wider font-semibold">Ubicación (opcional)</p>
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-brand-silver text-xs">Latitud</Label>
                <Input {...register('latitude')} className={fc} placeholder="ej. -13.1631" />
              </div>
              <div className="space-y-1.5">
                <Label className="text-brand-silver text-xs">Longitud</Label>
                <Input {...register('longitude')} className={fc} placeholder="ej. -72.5450" />
              </div>
            </div>
          </div>
